...
class Pdb(xbdb.Bdb, cmd.Cmd):
    ...
    def __init__(self, completekey='tab', stdin=None, stdout=None, skip=None):
        ...
        if 'HOME' in os.environ:
            ...
            try:
                rcFile = open(os.path.join(envHome, ".xpdbrc"))
            except IOError:
                pass
            else:
    ...
    def user_line(self, frame):
        ...
        """if there exist commands attach to this bp, will not enter interaction! """    
        if self.bp_commands(frame) or self.user_stopHereFlag:
            self.interaction(frame, None)
    ...
    def do_unsetWatch(self, arg):
        """ pdb command clear may clear watch point too """
        self.unset_watchPoint(arg)

    def do_infoWatchPoint(self, arg):
        """ display current watch point"""
        self.infoWatchPoint()
        
    do_infoWP = do_infoWatchPoint
    ...
    def help_finish(self):
        print >> self.stdout, """ jump nearest while or for loop in a function,
if does not have any loop, will stop at the caller of this function""" 
    ...
    def do_traceEveryLine(self, arg):
        self.set_traceEveryLine()

    do_tel = do_traceEveryLine
    
    def do_notTraceEveryLine(self, arg):
...
invalid n: will be ignore if n beyond the length of trace classes list"""
    ...
    def do_step(self, arg):
        ...
        if arg:
            arg += '-1'
            self.remainStepTimes = eval(arg) if eval(arg) > 0 else 0
        self.set_step()
        return 1
    do_s = do_step
    ...
    def do_next(self, arg):
        ...
        if arg:
            arg += '-1'
            self.remainNextTimes = eval(arg) if eval(arg) > 0 else 0
...
legally typed at the xpdb prompt.  Note!  You *can* override
internal xpdb commands with aliases!  Those internal commands
are then hidden until the alias is removed.  Aliasing is recursively
applied to the first word of the command line; all other words
in the line are left alone.

Some useful aliases (especially when placed in the .xpdbrc file) are:
...
def help():
    for dirname in sys.path:
        fullname = os.path.join(dirname, 'xpdb.doc')
        if os.path.exists(fullname):
